      ...
      case 'checkout.session.completed': {
        ...
        if (order) {
          ...
          const orderEmailData = {
            ...
            orderId: order.id,
            bookTitle: order.book_title || DEFAULT_BOOK_TITLE,
            bookFormat: order.book_format,
            quantity: order.quantity || 1,
            signingNames: order.signing_names || null,
            subtotal: subtotalAmount,
            taxAmount: taxAmount,
          ...
          if (customerEmail) {
            ...
            const bookTitle = session.metadata?.book_title || DEFAULT_BOOK_TITLE;
            ...
            const orderEmailData = {
              ...
              quantity: quantity,
              signingNames: session.metadata?.signing_names ? JSON.parse(session.metadata.signing_names) : null,
              subtotal: subtotalAmount,
              taxAmount: taxAmount,
